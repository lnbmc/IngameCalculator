        ...
        if (message.startsWith(".calc")) {
            executorService.submit(() -> {
                ...
                try {
                    ...
                    Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText(args.length == 1 ? calculate(args[0]) : "§cInvalid arguments."));
                } catch (ScriptException | IllegalArgumentException e) {
                    Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText("§cThis is not a valid expression."));
                }
            });
            return true;
        }
